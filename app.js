...
const authenticateToken = (request, response, next) => {
  ...
  const tweetAccessVerification =async (request, response, next) => {
      ...
      const{tweetId}= request.params;
       ...
       ON tweet.user_id= follower.following_user_id = '${userId}';`;
...
app.get("/user/tweets/feed/", authentication, async (request, response) => {
   ...
   WHERE
   user.user_id = '${followingPeopleIds}'
...
app.get("/user/followers/", authentication, async (request, response) => {
    const {username, userId}=request;
    const getFollowersQuery=`SELECT  DISTINCT name 
   FROM follower INNER JOIN user ON user.user_id=follower.follower.user_id
   WHERE
  follower.user_id ='${userId}';`;
 ...
 //API_6 
app.get("/tweets/tweetsId/", authentication, tweetAccessVerification, async (request, response) => {
    const {username, userId}=request;
     const {tweetId}=request.params;
...
//API_7
app.get("/tweets/tweetsId/likes/", authentication, tweetAccessVerification, async (request, response) => {
     const {tweetId}=request.params;
   ...
   const usersArray= likesUsers.map((eachUser) => each.user.username);
   response.send(likes: usersArray});
});   
//API_8
app.get("/tweets/tweetsId/replies/", authentication, tweetAccessVerification, async (request, response) => {
     const {tweetId}=request.params;
...
// API-10
    app.get("/user/tweets/", authentication, async (request, response) => {
     ...
     const createTweetQuery=`INSERT INTRO tweet(tweet.user_id, date_time)
...
//Delete  API-11
app.delete("/tweetss/:tweetId/",authentication, async (request, response) => {
  const { tweetId } = request.params;
  const {userId}=request;
